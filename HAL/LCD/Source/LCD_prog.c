...
void LCD_VoidSendStringFromPos (char *Ref_string, u8 Local_RowPosition, u8 Local_ColPosition)
{
	...
	{
		if((Local_RowPosition*16) + (Local_ColPosition + index) < Cols_Number)
		{
			LCD_VoidSendChar(Ref_string[index]);
			index++;
		}
		else if((Local_RowPosition*16) + (Local_ColPosition + index) == Cols_Number && line == Line_1)
		{
			LCD_VoidStartFrom(1,0);
			line++;
		}
		else if((Local_RowPosition*16) + (Local_ColPosition + index) < 32 && line == Line_2)
		{
			LCD_VoidSendChar(Ref_string[index]);
			index++;
		}
